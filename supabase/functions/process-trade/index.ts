...
serve(async (req) => {
  ...
  try {
    ...
    // Validate trade data
    if (!trade.account_id || !trade.security_id || !trade.quantity || !trade.trade_type) {
      return new Response(
    ...
    // Validate trade type
    const validTradeTypes = ['buy', 'sell', 'transfer_in', 'transfer_out', 'dividend', 'fee']
    if (!validTradeTypes.includes(trade.trade_type)) {
      return new Response(
        JSON.stringify({ error: 'Invalid trade type' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
