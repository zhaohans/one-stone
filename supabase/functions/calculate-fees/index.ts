...
serve(async (req) => {
  ...
  try {
    ...
    const { account_id, period_start, period_end, fee_type, fee_rate } =
      await req.json();
    ...
    switch (fee_type) {
      case "management":
        ...
        const { data: mgmtFee, error: mgmtError } = await supabaseClient.rpc(
      ...
      case "performance":
        ...
        if (positions) {
          ...
          const totalCost = positions.reduce(
            (sum, pos) => sum + pos.quantity * (pos.average_cost || 0),
            0,
          );
      ...
      case "transaction":
        ...
        if (trades) {
          calculatedAmount = trades.reduce(
      ...
      case "custody":
        ...
        if (custodyPositions) {
          const totalValue = custodyPositions.reduce(
            (sum, pos) => sum + (pos.market_value || 0),
            0,
          );
          ...
          calculatedAmount =
            ((totalValue * (fee_rate || 0.1)) / 100) * (days / 365);
          feeDescription = `Custody fee (${fee_rate || 0.1}% annually)`;
        }
        break;

      default:
        return new Response(JSON.stringify({ error: "Invalid fee type" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
    }
