...
import { extractTextFromPDF, extractTextFromDOCX } from '../services/extractors';
...
router.post('/upload', upload.single('file'), async (req: MulterRequest, res: Response) => {
  try {
    ...
    const fileName = isString(req.file.originalname) ? req.file.originalname : 'untitled';
...
router.post('/move/:id', async (req: Request, res: Response) => {
  try {
    ...
    await saveDocumentMetadata(id, { category: newCategory, driveFolderId: newDriveFolderId });
...
router.post('/:id/version', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    const fileName = typeof req.file.originalname === 'string' ? req.file.originalname : 'untitled';
    ...
    const newVersion = {
      driveFileId: driveFile.id,
      version: (docMeta?.versions?.length || 0) + 1,
      uploadedAt: new Date().toISOString(),
      uploader: 'TODO', // Replace with actual user info if available
      fileName,
    };
    const updatedVersions = [...(docMeta?.versions || []), newVersion];
...
router.get('/:id/versions', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const docMeta = await getDocumentMetadata(id);
    res.json({ success: true, versions: docMeta?.versions || [] });
  } catch (error) {
    res.status(500).json({ error: (error as Error).message });
  }
});
