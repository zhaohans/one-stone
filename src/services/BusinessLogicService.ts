...
export class BusinessLogicService {
  ...
  static async calculatePortfolioValue(accountId: string) {
    try {
      ...
      if (positions) {
        portfolio.totalValue = positions.reduce(
          (sum, pos) => sum + (pos.market_value || 0),
          0,
        );
        portfolio.totalCost = positions.reduce(
    ...
    try {
      ...
      positions.forEach((pos) => {
        ...
        sectorAllocation[sector] =
          (sectorAllocation[sector] || 0) + (pos.market_value || 0);
      });
      ...
      positions.forEach((pos) => {
        ...
        countryAllocation[country] =
          (countryAllocation[country] || 0) + (pos.market_value || 0);
      });
