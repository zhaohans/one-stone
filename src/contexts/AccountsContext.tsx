...
interface AccountsContextType {
  ...
  bulkUpdateAccounts: ReturnType<
    typeof useAccountOperations
  >["bulkUpdateAccounts"];
}
...
export const AccountsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const { createAccount, updateAccount, deleteAccount, bulkUpdateAccounts } =
  ...
  const fetchAccounts = async () => {
    ...
    try {
      const query = supabase
        .from("accounts")
        .select(
          `*, client:clients(id, first_name, last_name, client_code, email), positions(id, market_value)`,
        );
