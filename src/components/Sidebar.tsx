...
const Sidebar = ({ collapsed, onToggleCollapse }: SidebarProps) => {
  ...
  const { userStatus, profile, logout, user } = useAuth();
  ...
  const menuItems = [
    ...
    { icon: Receipt, label: "Fee Reports", path: "/fee-reports" },
  ...
  const sendErrorReport = async () => {
    ...
    await NotificationService.sendEmail({
      ...
      subject: `Sidebar Error Report: ${error.message}`,
  ...
  try {
    ...
    return (
      <ErrorPage
        code={500}
        message={error?.message || "Sidebar failed to render."}
