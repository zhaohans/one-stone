...
const DocumentsTable: React.FC = () => {
  ...
  const fetchDocs = async (q = '', tag = '', status = '', from = '', to = '', category = '', compliance = '') => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (data.success) {
        ...
        if (compliance === 'needs_attention') {
          docs = docs.filter((d: any) => d.complianceStatus && d.complianceStatus !== 'ok');
        }
        setDocuments(docs);
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fetchDocs(e.target.value, tagFilter, statusFilter, fromDate, toDate, categoryFilter, complianceFilter);
  };

  const handleTagFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setTagFilter(e.target.value);
    fetchDocs(search, e.target.value, statusFilter, fromDate, toDate, categoryFilter, complianceFilter);
  };

  const handleStatusFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setStatusFilter(e.target.value);
    fetchDocs(search, tagFilter, e.target.value, fromDate, toDate, categoryFilter, complianceFilter);
  };

  const handleFromDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFromDate(e.target.value);
    fetchDocs(search, tagFilter, statusFilter, e.target.value, toDate, categoryFilter, complianceFilter);
  };

  const handleToDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setToDate(e.target.value);
    fetchDocs(search, tagFilter, statusFilter, fromDate, e.target.value, categoryFilter, complianceFilter);
  };

  const handleCategoryFilter = (cat: string) => {
    setCategoryFilter(cat);
    fetchDocs(search, tagFilter, statusFilter, fromDate, toDate, cat, complianceFilter);
  };

  const handleComplianceFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setComplianceFilter(e.target.value);
    fetchDocs(search, tagFilter, statusFilter, fromDate, toDate, categoryFilter, e.target.value);
  };
  ...
  const handleSaveTags = async (doc: DocumentMeta, idx: number) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setDocuments(docs => docs.map((d, i) => i === idx ? { ...d, aiTags: { tags: editTags } } : d));
        setEditIdx(null);
      } else {
    ...
    try {
      if (moveCategory === '__new__' && moveFolderId) {
        ...
        if (data.success) {
          ...
          toast({ title: 'Folder created', description: `Folder "${moveFolderId}" created in Drive.` });
        } else {
          toast({ title: 'Error', description: data.error || 'Failed to create folder', variant: 'destructive' });
          setMoveLoading(false);
          return;
        }
      }
      ...
      if (data.success) {
        closeMoveModal();
        fetchDocs(search, tagFilter, statusFilter, fromDate, toDate, categoryFilter, complianceFilter);
        toast({ title: 'Document moved', description: 'Document moved successfully.' });
      } else {
        toast({ title: 'Error', description: data.error || 'Failed to move document', variant: 'destructive' });
      }
    } catch (err: any) {
      toast({ title: 'Error', description: err.message || 'Failed to move document', variant: 'destructive' });
    } finally {
      setMoveLoading(false);
    }
  };
  ...
  const handleVersionHistory = async (idx: number) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({ title: 'Error', description: data.error || 'Failed to fetch versions', variant: 'destructive' });
      }
    } catch (err: any) {
      toast({ title: 'Error', description: err.message || 'Failed to fetch versions', variant: 'destructive' });
    } finally {
  ...
  const handleUploadVersion = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({ title: 'Error', description: data.error || 'Failed to upload version', variant: 'destructive' });
      }
    } catch (err: any) {
      toast({ title: 'Error', description: err.message || 'Failed to upload version', variant: 'destructive' });
    } finally {
  ...
  const handleRestoreVersion = async (version: any) => {
    ...
    try {
      const res = await fetch(`/documents/${doc.id}/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driveFileId: version.driveFileId, fileName: version.fileName }),
      });
      const data = await res.json();
      if (data.success) {
        toast({ title: 'Version restored', description: 'This version is now the current version.' });
        fetchDocs(search, tagFilter, statusFilter, fromDate, toDate, categoryFilter, complianceFilter);
        handleVersionHistory(versionIdx); // Refresh versions
      } else {
      ...
      toast({ title: 'Error', description: err.message || 'Failed to restore version', variant: 'destructive' });
    }
  };
      ...
      <div className="flex-1">
        <div className="flex items-center gap-4 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input type="date" value={fromDate} onChange={handleFromDateChange} className="w-32" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs text-gray-500">To</label>
            <Input type="date" value={toDate} onChange={handleToDateChange} className="w-32" />
          </div>
        </div>
              ...
              {documents.map((doc, idx) => (
                <tr key={doc.id} className="border-b">
                  ...
                  <td className="p-2">
                    {editIdx === idx ? (
                      <div className="flex flex-wrap gap-2">
                        {editTags.map((tag, i) => (
                          <span key={i} className="flex items-center gap-1 bg-blue-100 px-2 py-1 rounded">
                            <Input
                              className="w-20 text-xs"
                              value={tag}
                              onChange={e => handleTagChange(i, e.target.value)}
                            />
                            <Button size="sm" variant="ghost" onClick={() => handleRemoveTag(i)}>
                              ×
                            </Button>
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(doc.aiTags?.tags || []).map((tag: string, i: number) => (
                          <Badge key={i} variant="secondary" className="text-xs">{tag}</Badge>
                        ))}
                      </div>
                    )}
                  </td>
                  <td className="p-2">{doc.uploadedAt ? new Date(doc.uploadedAt).toLocaleString() : ''}</td>
                  ...
                  <td className="p-2">
                    {editIdx === idx ? (
                      <>
                        <Button size="sm" onClick={() => handleSaveTags(doc, idx)}>Save</Button>
                        <Button size="sm" variant="ghost" onClick={() => setEditIdx(null)}>Cancel</Button>
                      </>
                    ) : (
                      <>
                        <Button size="sm" variant="outline" onClick={() => handleEdit(idx)}>Edit Tags</Button>
                        <a
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleMove(idx)}>Move</Button>
                        <Button size="sm" variant="ghost" onClick={() => handleVersionHistory(idx)}>Version History</Button>
                      </>
                    )}
                  </td>
                </tr>
        ...
        {moveIdx !== null && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50" role="dialog" aria-modal="true">
            <div className="bg-white p-6 rounded shadow-lg w-80">
              ...
              <div className="mb-2">
                <label className="block text-xs mb-1">Select Category/Folder</label>
                <select
                  value={moveCategory}
              ...
              <div className="flex gap-2 mt-4">
                <Button size="sm" onClick={handleMoveConfirm} disabled={moveLoading} aria-label="Confirm move">
                  {moveLoading ? <span className="animate-spin mr-2">⏳</span> : null}
                  {moveLoading ? 'Moving...' : 'Move'}
                </Button>
                <Button size="sm" variant="ghost" onClick={closeMoveModal} aria-label="Cancel move">Cancel</Button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white p-6 rounded shadow-lg w-[32rem] max-h-[90vh] overflow-y-auto">
              ...
              {versionLoading ? (
                ...
                <>
                    ...
                    <tbody>
                      {versions.map((v, i) => (
                        <tr key={i} className="border-b">
                          ...
                          <td className="p-2">{v.uploadedAt ? new Date(v.uploadedAt).toLocaleString() : ''}</td>
                  ...
                  <div className="mt-4">
                    <label className="block text-xs mb-1">Upload New Version</label>
                    <Input
                      type="file"
                      accept=".pdf,.docx"
              ...
              <div className="flex gap-2 mt-4">
                <Button size="sm" variant="ghost" onClick={closeVersionModal} aria-label="Close version history">Close</Button>
              </div>
            </div>
          </div>
        )}
...
function getComplianceBadge(status?: string) {
  switch (status) {
    case 'missing_tags':
      return <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Missing Tags</span>;
    case 'missing_expiry':
      return <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Missing Expiry</span>;
    case 'missing_status':
      return <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Missing Status</span>;
    case 'expiring_soon':
      return <span className="bg-orange-100 text-orange-800 px-2 py-1 rounded text-xs">Expiring Soon</span>;
    default:
      return <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">OK</span>;
  }
}
