    ...
    try {
      ...
      if (
        ...
        if (validStatuses.includes(filters.kyc_status as any)) {
          query = query.eq(
            "kyc_status",
            filters.kyc_status as (typeof validStatuses)[number],
          );
        }
      }
      if (filters.country) {
      ...
      if (filters.search) {
        ...
        query = query.or(
          `first_name.ilike.${searchTerm},last_name.ilike.${searchTerm},email.ilike.${searchTerm},client_code.ilike.${searchTerm}`,
        );
      }
